...
exports.postAPI = (req, res) => {
  ...
  res.status(201).json({
    error: false,
    message: `buku dengan id ${id} berhasil dibuat!`,
  });
};
...
exports.getAPIByIdMahasiswa = (req, res) => {
  const mhs = mahasiswa.find((i) => i.id_mahasiswa === +req.params.id);
  if (mhs !== undefined) {
...
exports.deleteAPIMahasiswa = (req, res) => {
  ...
  res.status(200).json({
    message: `Mahasiswa dengan id ${req.params.id} berhasil dihapus!`,
  });
};
